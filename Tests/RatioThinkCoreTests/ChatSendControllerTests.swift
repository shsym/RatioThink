...
final class ChatSendControllerTests: XCTestCase {
  ...
  /// engine-true occupancy, NOT this turn's token count. It must publish
  /// through `onUsage` for the live meter but must NOT be written into
  /// `Message.tokens` — that field keeps one consistent per-message
  /// meaning regardless of write path (here: untouched at its default 0,
  /// since the engine reports no per-message token count for this turn).
  func test_usage_frame_publishes_occupancy_but_does_not_write_message_tokens() async throws {
    let container = try RatioThinkModelContainer.makeInMemory()
    let context = ModelContext(container)
    let chat = Chat()
    context.insert(chat)
    ...
    // But it must reach the live meter.
    XCTAssertEqual(published, ContextUsage(usedTokens: 4242, windowTokens: 8192))
  }
