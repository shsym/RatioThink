...
the full signing recipe), then launch /Applications/Rational.app and
complete the first-launch wizard so SMAppService.agent registers the plist.
Re-run this script with Rational.app running.
EOF
    exit 2
  fi
}
...
SKIP: Rational.app is not running. The #412 runtime recovery is driven BY
the running App (it polls the helper, detects sustained unreachability, and
re-registers the launchd job). Launch /Applications/Rational.app and re-run.
EOF
