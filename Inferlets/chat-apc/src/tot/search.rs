...
/// forward pass that the level-1 spin fix removed. `model` drives the
/// per-node reasoning/chat demux ([`generate_demuxed`]) + the value scorer.
pub async fn run(root_ctx: Context, params: &TotParams, model: &Model) -> SearchOutcome {
...
/// an `ok`/`incomplete`/`error` leaf; an `ok` leaf may still carry a
/// `score_error` when the scorer infra failed (F4). Pruning reuses
/// [`select_beam`], which keeps only the top `beam_width` **ok** candidates.
/// Node ids are caller-assigned (paired with the engine contexts), so the
/// returned `keep` ids map straight back to surviving [`Frontier`] entries.
