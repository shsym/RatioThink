...
/// What `tot.wasm` returns to the gateway.
///
/// NOT the wire response. chat-apc's `TreeResponse` carried `id` and
/// `object` — both gateway-owned now (§3), and a guest-minted `id` is exactly
/// the `bon-0` collision bug, since a per-request wasm instance restarts its
/// counter every time.
///
/// `model` IS carried, and is the resolved, membership-checked engine id: it is
/// the string hashed into the `conv/` boundary name, so surfacing it is how a
/// test proves ToT and chat digested the same one. `boundary_found` /
/// `reused_tokens` are here for the same reason — without them the B2 exit test
/// could only observe that ToT *saved* a boundary, never that it *reused* one,
/// so a ToT path that cold-starts every turn would pass.
...
/// Node id minter, owned by one search run.
///
/// Replaces chat-apc's process-global `AtomicU64` (`tot/tree.rs:136-143`), for
/// two reasons:
///
/// * **The tree id is gone.** `new_tree_id()` drew from this same counter, but
///   the response id is gateway-owned now (`ratio-wire/src/event.rs:89-96`) —
///   a per-request wasm instance restarts its counter every time, which is
///   exactly the `bon-0` collision bug.
/// * **A global is wrong under per-request instantiation** anyway: it would
///   reset to 0 each run, so nothing is gained by keeping it static.
///
/// SEEDED AT 1, not 0. chat-apc's first node is `tot-n1` only because
/// `new_tree_id()` consumed value 0 first (`tot/mod.rs:338`). A fresh counter
/// starting at 0 would emit `tot-n0` and change every id in the captured
/// stream, breaking the golden comparison for a purely cosmetic reason.
///
/// Ids are minted on the sequential level loop before branches are spawned
/// (all `node_start`s precede any delta), so `&mut self` is not a constraint
/// on concurrency.
#[derive(Debug)]
pub struct NodeIds {
    next: u64,
...
/// `id` is passed in rather than minted here: the caller already minted it to
/// emit `node_start`, and a failed branch must complete the SAME node the
/// client is rendering, not a new one.
pub fn error_leaf(
    id: String,
    parent_id: &str,
    depth: usize,
...
/// `threshold`, see [`crate::diversity`]) the lower-scored paraphrase is
...
pub fn select_beam_diverse(candidates: &[Candidate], m: usize, threshold: f32) -> Vec<String> {
    ...
    for c in ranked {
        ...
        let dup = kept
            .iter()
            .any(|k| crate::diversity::is_near_duplicate(&k.content, &c.content, threshold));
        if dup {
            deferred.push(c);
        } else {
            kept.push(c);
...
mod tests {
    ...
    fn diverse_beam_demotes_paraphrase_for_a_distinct_branch() {
        ...
        let keep = select_beam_diverse(&c, 2, crate::diversity::DUP_THRESHOLD);
    ...
    fn diverse_beam_matches_plain_beam_when_all_distinct() {
        ...
        assert_eq!(
            select_beam_diverse(&c, 2, crate::diversity::DUP_THRESHOLD),
            vec!["c", "d"]
        );
    }
    ...
    fn diverse_beam_excludes_non_ok_candidates() {
        ...
        assert_eq!(
            select_beam_diverse(&c, 3, crate::diversity::DUP_THRESHOLD),
            vec!["ok1", "ok2"]
        );
    }
    ...
    fn error_leaf_is_wire_visible_and_beam_excludable() {
        let n = error_leaf("tot-n9".into(), "p1", 2, 1, "fork failed: boom".to_string());
    ...
    #[test]
    fn result_serializes_expected_keys_and_round_trips() {
        // `id` and `object` are DELIBERATELY absent: the gateway owns response
        // identity now, and a guest-minted id is the `bon-0` collision bug.
        let resp = TreeResult {
            model: "m".to_string(),
            breadth: 3,
            depth: 2,
        ...
        for k in ["id", "object"] {
            assert!(v.get(k).is_none(), "result must not mint {k}; the gateway owns it");
        }
        let root = v.get("root").unwrap();
        for k in ["id", "parent_id", "depth", "branch_index", "content", "score", "status", "children"] {
            assert!(root.get(k).is_some(), "root missing key {k}");
        }
    ...
    fn node_reasoning_serializes_when_present_omitted_when_empty() {
        let mut n = error_leaf("tot-n9".into(), "root", 1, 0, "x".to_string());
