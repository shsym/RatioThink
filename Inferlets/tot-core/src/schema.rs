...
pub struct TotInput {
    ...
    /// chat-apc entirely — legacy ToT has no `Context::save`/`open` and no
    /// prefix-cache reference, so every ToT request re-prefills the whole
    /// history. This field is what lets a ToT turn open the boundary a chat
    /// turn left, and leave one a chat turn can open.
