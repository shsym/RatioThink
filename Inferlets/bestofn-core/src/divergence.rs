...
/// [`crate::schema::MAX_N`] (5), so the five stances cover every sibling
...
pub fn candidate_directive(branch_index: usize, thinking: bool) -> String {
...
pub fn candidate_retry_directive(branch_index: usize) -> String {
    format!(
...
pub fn requires_stochastic_sampling(temperature: f32) -> bool {
    temperature > 0.0
}
...
mod tests {
    use super::*;
    use crate::schema::{DEFAULT_N, MAX_N};
    ...
    fn stochastic_sampling_guard_tracks_temperature() {
        assert!(
            requires_stochastic_sampling(
                crate::schema::resolve(&crate::schema::BestOfNInput::default())
                    .unwrap()
                    .temperature
            ),
