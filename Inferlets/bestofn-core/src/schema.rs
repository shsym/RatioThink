...
pub struct BestOfNInput {
    ...
    /// Conversation identity for the mode-neutral KV boundary. Absent from
    /// chat-apc entirely — its Best-of-N writes zero `conv/` names, so a
    /// committed round left nothing for the next chat turn to reuse.
    pub boundary: Option<gen_core::boundary::BoundaryDirective>,
    /// Gateway-minted round identity, echoed back by the client from
    /// `tree_start.id`. THE authorization scope for every delete this round
    /// performs: chat-apc derived round ids from a process counter that
    /// restarts with each wasm instance, so every round of every chat was
    /// `bon-0` and a round id authorized nothing.
    pub round_id: Option<String>,
    /// Terminal commit: save the selected answer's `conv/` boundary, THEN free
    /// the round. Absent on a generative request.
...
/// The §6.2 commit operation's payload.
#[derive(Debug, Clone, Deserialize, Default)]
pub struct CommitInput {
    /// The candidate the user accepted. Must belong to `round_id`.
    pub snapshot_name: Option<String>,
    /// The exact visible text the client persisted as the assistant message.
    /// The `conv/` boundary is built from `messages + assistant(answer)`, so
    /// this must be byte-identical to what the next chat turn will resend.
    pub answer: Option<String>,
    /// Every snapshot this round minted, including the selected one. Each is
    /// checked against `round_id` before it can reach `Context::delete`.
    #[serde(default)]
