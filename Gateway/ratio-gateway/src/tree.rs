...
//!
//! The migration plan originally claimed a new inferlet would need "no gateway
//! code". That was false, and this file is the proof: a tree stream differs
//! from a chat stream in almost every structural decision the chat driver
//! hardcodes. Read off the captured stream
//! (`Inferlets/ratio-wire/tests/fixtures/tot_stream.sse`, 39 frames):
//!
//! | | chat-v1 | tree-v1 |
//! |---|---|---|
...
//!
//! Two of those are silent failures rather than loud ones, which is why they
//! are called out here:
//!
//! * **A tree stream contains no `Ready`.** Pointing the chat driver at a tree
//!   inferlet does not error — it blocks until `first_event_timeout` and then
//!   504s, because the deferred-commit loop breaks only on `Ready`.
//! * **`terminal()` unconditionally emits an OpenAI finish chunk**
//!   (`render.rs:352`). No tree capture contains one, so calling it appends a
//!   frame that is not in the protocol. This driver never calls it.
//!
//! What tree-v1 does NOT need is a new renderer: `OpenAiSse` already renders
//! every tree variant, because the `Event` vocabulary was declared in full
//! (including the tree events no producer emitted yet) before ToT was ported.
//! That was the anti-rewrite lever and it paid off.
...
mod tests {
    ...
    /// Best-of-N ends without a `tree_complete` at all — it hands the client a
    /// pick set and stops. Recognizing it now is what keeps milestone C a
    /// producer change rather than a protocol change.
    #[test]
    ...
    /// The whole point of the class: a tree stream must never carry chat's
    /// opening or closing frames.
    #[test]
