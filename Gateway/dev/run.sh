...
cmd_crossmode() {
  ...
  #
  # Reasoning-only regression: `crossmode.py` deliberately keeps a small
  # MAX_TOKENS default. A Qwen3.x-class model may return content="" with all
  # output in `reasoning_content`; the harness mirrors the app by excluding the
  # empty assistant row. The entry half must still reuse chat's prompt-only
  # checkpoint. Raising MAX_TOKENS is useful for answer-quality debugging, but
  # must not be required for this cache assertion to pass.
...
cmd_bon() {
  ...
  #
  # Same reasoning-only regression as crossmode: an empty visible chat answer
  # is omitted from history, and Best-of-N must enter through the prompt-only
  # checkpoint rather than requiring hidden reasoning in `conv/`.
