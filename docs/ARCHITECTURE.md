...
| `startEngine(profileID:)` / `stopEngine()` | Bring the engine up for a profile / shut it down. |
...
**Cold start.** App launches → reconciles the helper registration (re-registers via
`SMAppService` if the mach service is unreachable) → calls `startEngine(profileID:)`
→ the helper's `PieControlLauncher` boots `pie serve`, completes the WS handshake,
and installs `chat-apc` → engine reports `.running(port:)` → the app's
`EngineStatusStore` (polling at ~1 Hz) sees it and resolves the HTTP base URL.
...
- **XPC status is pull, not push.** The app polls `engineStatus()`; the helper does not
  stream changes. Every XPC reply carries an error channel so a dropped click is never
  silently swallowed (`Shared/XPC/PieHelperXPC.swift`).
