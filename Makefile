...
ci-pr: lint check-vendor-pin test-ci-v2-static-gate test-lint-gui-only-testing test-assert-gui-tests-executed test-lint-e2e-gui-gating verify-app-icon-assets test-app-icon-assets verify-docs-icons test-docs-icons test-menubar-icon-template test-landing-page build-static test-unit test-inferlets test-apc-bench-selftest test-install-guards test-collect-diagnostics test-sanitizer-canary test-release ## Lightweight local/manual gate: static/lint/provenance + compile/type + deterministic unit/contracts including release scripts
...
local-e2e-gate: test-e2e-engine test-e2e-models test-e2e-chat test-e2e-tot test-e2e-budget-sweep test-e2e-full test-e2e-cache-real test-e2e-apc-divergence test-e2e-bestofn-regen bench-apc-real test-gui-history test-gui-stream-cancel test-gui-chat-retry test-gui-load-default test-gui-first-launch-package test-e2e-package test-helper-respawn test-helper-recovery test-quit-structured ## Operator-gated integration/E2E parity; requires documented models, engine, signing, TCC, or live services
...
test-gui-shell: genproject $(LOGDIR) ## GUI area: app window shell + Settings 4 tabs/profiles + ratiothink://settings deep link (S5/S420/S723)
	$(call gui_suite_run,shell,-only-testing:RatioThinkGUITests/S5_AppWindowShellGUITests -only-testing:RatioThinkGUITests/S420_SettingsDeepLinkGUITests -only-testing:RatioThinkGUITests/S723_ProfileDescriptionsGUITests)
