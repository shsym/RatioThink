...
| `S507_StreamContinuityGUITests` | chat send/persist | switch chats mid-stream → stream survives (no cancel), sidebar row shows streaming indicator, held stream released in background finishes + persists, composer live after | app+fake-engine (holding SSE + /control/release) | `test-gui-stream-cancel` |
